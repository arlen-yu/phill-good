...
def crime_map(locations, hr):
	...
	if (hr >= 22 or hr <= 2):
		shit = c.execute("SELECT crime FROM data WHERE (x >= ? AND  x <= ?) AND (y >= ? AND y <= ?) AND (hour >= ? OR hour <= ?)", 
		(locations[0], locations[1], locations[2], locations[3], minhr, maxhr))
	else:
		shit = c.execute("SELECT crime FROM data WHERE (x >= ? AND  x <= ?) AND (y >= ? AND y <= ?) AND (hour >= ? AND hour <= ?)", 
		(locations[0], locations[1], locations[2], locations[3], minhr, maxhr))
